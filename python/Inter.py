...
temp_adata = sc.read_csv("/home/yuanjiaxin/ccc/hpancreas/hpancreas1YS.csv", first_column_names=True)
temp_adata =temp_adata.T
temp_adata.var_names = [str(i).upper() for i in temp_adata.var_names]
temp_adata.var_names_make_unique()
        
train_adata = temp_adata
train_adata.var_names_make_unique()
    
Trainadata_num = len(train_adata)
#Getting the testing dataset and making var names unique
test_adata = sc.read_csv( "/home/yuanjiaxin/ccc/xin/xinpancreas1YS.csv")
test_adata =test_adata.T
test_adata.var_names = [str(i).upper() for i in test_adata.var_names]
...
def getXY(gap, adata, Traindata_paths):
    ...
    if(Traindata_paths != None):
        ...
        y_trains = []
        y_train = pd.read_csv(Traindata_paths + "Labels.csv")
        y_train = y_train.T
...
train_data, train_cells, train_cellTypes = getXY(gap=1024, adata=Train_adata, Traindata_paths="/home/yuanjiaxin/ccc/hpancreas/")
#Converting the preprocessed test dataset into the input of Transformer Encoder 
test_data ,test_cells, test_cellTypes= getXY(gap=1024, adata=Test_adata, Traindata_paths= "/home/yuanjiaxin/ccc/xin/")
...
class scTransformer(nn.Module):
    ...
    def forward(self, mels):
        ...
        return out
gap = 1024          #the length of a sub-vector
...
dp = 0.1            #dropout rate
batch_sizes = 640  #the size of batch

n_epochs = 50     #the number of epoch
   
#Getting the data which input into the scTransformer
train_data, labels, query_data, cell_types,labels2 = train_data,Train_labels,test_data,cell_types,Test_labels
#Number of cell types plus unassigned cell type
num_classes =len( np.unique(cell_types)) + 1
...
test_dataset = TrainDataSet(data=query_data, label=labels2)
...
#startting training scTransformerUsing training data to train scTransformer
